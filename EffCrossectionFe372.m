function [sigma_eff] = EffCrossectionFe372(freqlaser,temp,wind)
...
SigmaD = sqrt(kB*temp./mFeMean/(Lambda_Center_Fe372^2));
Sigmae1 = sqrt(SigmaD.^2 + PAL372RMSwidth(1)^2);
Sigmae2 = sqrt(SigmaD.^2 + PAL372RMSwidth(2)^2);
% compute effective cross-section
comon1 = fosc_Fe372*Qe^2/(sqrt(2*pi)*4*E0*Me*c)./Sigmae1;
comon2 = fosc_Fe372*Qe^2/(sqrt(2*pi)*4*E0*Me*c)./Sigmae2;
sigma_eff1 = comon1.*exp(-(freq - wind/Lambda_Center_Fe372).^2/2./(Sigmae1.^2));
                % portion determined by the narrow peak
sigma_eff2 = comon2.*exp(-(freq - wind/Lambda_Center_Fe372).^2/2./(Sigmae2.^2));
                % portion determined by the pedestal
sigma_eff = PAL372Eportion(1)*sigma_eff1 + PAL372Eportion(2)*sigma_eff2;
                % total effective cross-section contributed by the narrow
                % peak and the pedestal, 
                % PAL372Eportion(1) + PAL372Eportion(2) = 1 by definition
