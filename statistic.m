...
function [SNR_std, SNR_mean,SNR_min,SNR_max] = statistic(year,month,day,ScreenRawData) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
OverviewRawData = 1;
                  % 0 = no screening, 1 = do raw data screening
...
Process4DailyMean = 1;      % Formal processing routine #3 for daily temperature or long-integration temperature
SaveDailymeanData = 1;
...
%BG_start_alt = 165e3;             % unit: m for all altitudes
BG_end_alt = 180e3;
BG_start_alt2 = 70e3; 
BG_end_alt2 = 75e3;
...
Rayleigh_sum_start_alt = 30.0e3;
Rayleigh_sum_end_alt = 45.0e3;
Rayleigh_norm_alt = 37.5e3;
Rayleigh_fit_start_alt = 45e3;
Rayleigh_fit_end_alt = 55e3;
Rayleigh_fit_alt = 50.0e3;
% Fe_start_alt = 75e3;
...
goodfilescount_374 = 1;
goodfilescount_372 = 1;
    for system = 1:2
        ...
        for dd = 1:length(day)
            ...
            %         end
            if (ScreenRawData == 0)
%                 goodfilelist = textread(sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/GoodfilelistNew',year,smonth(month,:),day(dd),system),'%s');
%                 Numofprofile(dd) = size(goodfilelist,1); clear goodfilelist
                fp = fopen(sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/Goodfilelist.txt',year,smonth(month,:),day(dd),system),'r');
                Numofprofile(dd) = fscanf(fp,'%d',[1]);
            elseif (ScreenRawData == 1)
                fp = fopen(sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/filelist.txt',year,smonth(month,:),day(dd),system),'r');
                Numofprofile(dd) = fscanf(fp,'%d',[1]);
            end
...
%             end
            ...
            for mm = 1:Numofprofile(dd)
                ...
                fpin = fopen([sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/%s',year,smonth(month,:),day(dd),system,nameofprofile(nn,:))],'r');
                ...
                %Chop off data greater than 24 hours
                if timePast23 ==1 && timeraw(nn) < 1
                    timeraw(nn)=[];
                    fclose(fpin);
                    break
...
%                 BaseAlt/1000; % unit: km   
                ...
                N_tot = rawphotoncnt(BinRangeNum(1):BinRangeNum(2));
                N_b = Bkg(nn);
                N_sig = N_tot-N_b;
                SNR(nn) = sum(N_sig)/sum(sqrt(N_tot));
                
               %Save only data sets based on SNR criteria
             if ScreenRawData == 1
               if SNR(nn) > 8.2 && system == 1
                   goodfileslist_374(goodfilescount_374,:) = nameofprofile(nn,:);
                goodfilescount_374 =  goodfilescount_374+1;
               end
                   
                 
               if SNR(nn) > 2.5 && system == 2
                   goodfileslist_372(goodfilescount_372,:) = nameofprofile(nn,:);
                goodfilescount_372 =  goodfilescount_372+1;
               end
...
%         %Clear the data after midnight
%                 maxval = max(timeraw);
%                 for i = 1:10
%                     if timeraw(end-i+1) == maxval
%                         break;
%                     end
%                     if timeraw(end-i+1) < maxval
%                         Datainfo(end-i+1,:)=[];
...
%% Compute statistics
        SNR_std(1,system) = std(SNR);
        SNR_mean(1,system) = mean(SNR);
        SNR_min(1,system) = min(SNR);
        SNR_max(1,system) = max(SNR);
        %For text location purposes
        timeraw_max = max(timeraw);
        timeraw_min = min(timeraw);
        location_text_x = timeraw_min+0.5;
        location_text_y = SNR_max-1;
        ...
        if (system == 2)
            ...
            text(location_text_x,16,sprintf('STD = %4.2f\n mean = %4.2f\n min = %4.2f\n max = %4.2f\n',SNR_std(1,system),SNR_mean(1,system),SNR_min(1,system),SNR_max(1,system)))
        elseif (system == 1)
        ...
        if (ScreenRawData == 1)
                    if (system == 1)
                        saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighStatistics/FIG/%s%04d%s%02dstat374nmNew.fig',station,station,year,smonth(month,:),day(1)));
                        print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighStatistics/EPS/%s%04d%s%02dstat374nmNew',station,station,year,smonth(month,:),day(1)));
                        print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighStatistics/JPG/%s%04d%s%02dstat374nmNew',station,station,year,smonth(month,:),day(1)));
                    elseif (system == 2)
                        saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighStatistics/FIG/%s%04d%s%02dstat372nmNew.fig',station,station,year,smonth(month,:),day(1)));
                        print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighStatistics/EPS/%s%04d%s%02dstat372nmNew',station,station,year,smonth(month,:),day(1)));
                        print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighStatistics/JPG/%s%04d%s%02dstat372nmNew',station,station,year,smonth(month,:),day(1)));
                    end
                elseif (ScreenRawData == 0)
                    if (system == 1)
                        saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighGoodStatistics/FIG/%s%04d%s%02dstat374nmGoodNew.fig',station,station,year,smonth(month,:),day(1)));
                        print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighGoodStatistics/EPS/%s%04d%s%02dstat374nmGoodNew',station,station,year,smonth(month,:),day(1)));
                        print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighGoodStatistics/JPG/%s%04d%s%02dstat374nmGoodNew',station,station,year,smonth(month,:),day(1)));
                    elseif (system == 2)
                        saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighGoodStatistics/FIG/%s%04d%s%02dstat372nmGoodNew.fig',station,station,year,smonth(month,:),day(1)));
                        print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighGoodStatistics/EPS/%s%04d%s%02dstat372nmGoodNew',station,station,year,smonth(month,:),day(1)));
                        print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/RayleighGoodStatistics/JPG/%s%04d%s%02dstat372nmGoodNew',station,station,year,smonth(month,:),day(1)));
                    end
        end
...
%% Create Goodfilelists for both 374nm and 372nm channels
if ScreenRawData == 1
 system = 1;
 fpout = fopen(sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/Goodfilelist.txt',year,smonth(month,:),day(dd),system),'w');
 fprintf(fpout,'%d \n',goodfilescount_374-1);
 for i = 1: goodfilescount_374-1
 fprintf(fpout,[goodfileslist_374(i,:) '\n']);
 end
 fclose(fpout);   
 ...
 fpout = fopen(sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/Goodfilelist.txt',year,smonth(month,:),day(dd),system),'w');
 fprintf(fpout,'%d \n',goodfilescount_372-1);
 for i = 1: goodfilescount_372-1
fprintf(fpout,[goodfileslist_372(i,:) '\n']);
 end
 fclose(fpout);  
...
%                 if (ScreenRawData == 1)
%                     if (system == 1)
%                         saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/FIG/%s%04d%s%02dstat374nmNew.fig',station,station,year,smonth(month,:),day(1)));
%                         print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/EPS/%s%04d%s%02dstat374nmNew',station,station,year,smonth(month,:),day(1)));
%                         print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/JPG/%s%04d%s%02dstat374nmNew',station,station,year,smonth(month,:),day(1)));
%                     elseif (system == 2)
%                         saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/FIG/%s%04d%s%02dstat372nmNew.fig',station,station,year,smonth(month,:),day(1)));
%                         print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/EPS/%s%04d%s%02dstat372nmNew',station,station,year,smonth(month,:),day(1)));
%                         print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/JPG/%s%04d%s%02dstat372nmNew',station,station,year,smonth(month,:),day(1)));
%                     end
%                 elseif (ScreenRawData == 0)
%                     if (system == 1)
%                         saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/FIG/%s%04d%s%02dstat374nmGoodNew.fig',station,station,year,smonth(month,:),day(1)));
%                         print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/EPS/%s%04d%s%02dstat374nmGoodNew',station,station,year,smonth(month,:),day(1)));
%                         print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/JPG/%s%04d%s%02dstat374nmGoodNew',station,station,year,smonth(month,:),day(1)));
%                     elseif (system == 2)
%                         saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/FIG/%s%04d%s%02dstat372nmGoodNew.fig',station,station,year,smonth(month,:),day(1)));
%                         print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/EPS/%s%04d%s%02dstat372nmGoodNew',station,station,year,smonth(month,:),day(1)));
%                         print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/StatisticsPlots/JPG/%s%04d%s%02dstat372nmGoodNew',station,station,year,smonth(month,:),day(1)));
%                     end
%                 end
%             end
%             pause(0.1)
...
%                 end
%                 if (SaveStatData == 1)
%                     nametobesaved1 = sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/GoodfilelistNew',year,smonth(month,:),day(dd),system);
%                     dlmwrite(nametobesaved1,goodfilenames,'')
%                     nametobesaved2 = sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/BadfilelistNew',year,smonth(month,:),day(dd),system);
...
%                 [smdCnt(:,kk)] = HammingSmth(FWtime,time0,timeraw,processedCnt(:,kk));
%             end
%             figure
...
%                 [smdsmdCnt(:,kk)] = HammingSmth(FWtime,time0,timeraw,smdCnt(:,kk));
%             end
%             figure
%             colormap('jet')
...
%                 if (ScreenRawData == 1)
%                     if (system == 1)
%                         saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/FIG/%sFe374nm%04d%s%02dAllNew.fig',station,station,year,smonth(month,:),day(1)));
%                         print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/EPS/%sFe374nm%04d%s%02dAllNew',station,station,year,smonth(month,:),day(1)));
%                         print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/JPG/%sFe374nm%04d%s%02dAllNew',station,station,year,smonth(month,:),day(1)));
%                     elseif (system == 2)
%                         saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/FIG/%sFe372nm%04d%s%02dAllNew.fig',station,station,year,smonth(month,:),day(1)));
%                         print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/EPS/%sFe372nm%04d%s%02dAllNew',station,station,year,smonth(month,:),day(1)));
%                         print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/JPG/%sFe372nm%04d%s%02dAllNew',station,station,year,smonth(month,:),day(1)));
%                     end
%                 elseif (ScreenRawData == 0)
%                     if (system == 1)
%                         saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/FIG/%sFe374nm%04d%s%02dGoodNew.fig',station,station,year,smonth(month,:),day(1)));
%                         print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/EPS/%sFe374nm%04d%s%02dGoodNew',station,station,year,smonth(month,:),day(1)));
%                         print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/JPG/%sFe374nm%04d%s%02dGoodNew',station,station,year,smonth(month,:),day(1)));
%                     elseif (system == 2)
%                         saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/FIG/%sFe372nm%04d%s%02dGoodNew.fig',station,station,year,smonth(month,:),day(1)));
%                         print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/EPS/%sFe372nm%04d%s%02dGoodNew',station,station,year,smonth(month,:),day(1)));
%                         print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/JPG/%sFe372nm%04d%s%02dGoodNew',station,station,year,smonth(month,:),day(1)));
%                     end
%                 end
%             elseif (Process4temp == 1)
%                 if (system == 1)
%                     saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/FIG/%sFe374nm%04d%s%02dTempNew.fig',station,station,year,smonth(month,:),day(1)));
...
%                 elseif (system == 2)
%                     saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewPlots/FIG/%sFe372nm%04d%s%02dTempNew.fig',station,station,year,smonth(month,:),day(1)));
...
%                 if (ScreenRawData == 1)
%                     if (system == 1)
%                         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewData/%sFe374nm%04d%s%02dAllNew.txt',station,station,year,smonth(month,:),day(1));
%                     elseif (system == 2)
%                         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewData/%sFe372nm%04d%s%02dAllNew.txt',station,station,year,smonth(month,:),day(1));
%                     end
%                 elseif (ScreenRawData == 0)
%                     if (system == 1)
%                         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewData/%sFe374nm%04d%s%02dGoodNew.txt',station,station,year,smonth(month,:),day(1));
%                     elseif (system == 2)
%                         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewData/%sFe372nm%04d%s%02dGoodNew.txt',station,station,year,smonth(month,:),day(1));
%                     end
%                 end
%             elseif (Process4temp == 1)
%                 if (system == 1)
%                     filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewData/%sFe374nm%04d%s%02dTempNew.txt',station,station,year,smonth(month,:),day(1));
%                 elseif (system == 2)
%                     filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/OverviewData/%sFe372nm%04d%s%02dTempNew.txt',station,station,year,smonth(month,:),day(1));
%                 end
%             end
...
%             goodfilelist = textread(sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/GoodfilelistNew',year,smonth(month,:),day(dd),system),'%s');
%             Numofprofile(dd) = size(goodfilelist,1); clear goodfilelist
%             fp = fopen(sprintf('/Users/asta10/Desktop/Research/SkyObservations/%04d%s%02d/RX%d/GoodfilelistNew',year,smonth(month,:),day(dd),system),'r');
%             for ii=1:Numofprofile(dd)
%                 if (dd > 1)
%                     jj = ii + sum(Numofprofile(1:dd-1));        % for handling multiple days of data
%                 elseif (dd == 1)
...
%                     nn = mm;
%                 end
%                 fpin = fopen([sprintf('/Users/asta10/Desktop/Research/ScienceProjects/SkyObservations/%04d%s%02d/RX%d/%s',year,smonth(month,:),day(dd),system,nameofprofile(nn,:))],'r');
%                 fgetl(fpin); fgetl(fpin); fgetl(fpin); fgetl(fpin); fgetl(fpin);
...
%         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/ProcessedData/%sPreproFeCnt374nm%04d%s%02dNew.txt',station,station,year,smonth(month,:),day(1));          
%         save(filename,'datatobesaved','-ASCII')
%         clear datatobesaved
%         datatobesaved = [355 DataInfo2(:,4)';altitude(Fe_start_bin:Fe_end_bin)' PuresigFe372(:,Fe_start_bin:Fe_end_bin)'];
%         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/ProcessedData/%sPreproFeCnt372nm%04d%s%02dNew.txt',station,station,year,smonth(month,:),day(1));
%         save(filename,'datatobesaved','-ASCII')
%         clear datatobesaved
%         datatobesaved = [355 DataInfo1(:,4)';altitude(Fe_start_bin:Fe_end_bin)' Normdsumsig374(:,Fe_start_bin:Fe_end_bin)'];
%         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/ProcessedData/%sPreproNormCnt374nm%04d%s%02dNew.txt',station,station,year,smonth(month,:),day(1));          
%         save(filename,'datatobesaved','-ASCII')
%         clear datatobesaved
%         datatobesaved = [355 DataInfo2(:,4)';altitude(Fe_start_bin:Fe_end_bin)' Normdsumsig372(:,Fe_start_bin:Fe_end_bin)'];
%         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/ProcessedData/%sPreproNormCnt372nm%04d%s%02dNew.txt',station,station,year,smonth(month,:),day(1));
%         save(filename,'datatobesaved','-ASCII')
%         clear datatobesaved
%     end
% 
...
%         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/ProcessedData/%sSmoothFeCnt374nm%04d%s%02dNew.txt',station,station,year,smonth(month,:),day(1));          
%         save(filename,'datatobesaved','-ASCII')
%         clear datatobesaved
%         datatobesaved = [355 timebinsmooth;altbinsmooth(Fe_start_bin:Fe_end_bin)' smdsmdPureFe372(:,Fe_start_bin:Fe_end_bin)'];
%         filename=sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/ProcessedData/%sSmoothFeCnt372nm%04d%s%02dNew.txt',station,station,year,smonth(month,:),day(1));
%         save(filename,'datatobesaved','-ASCII')
%         clear datatobesaved
%     end
%     hold off
...
%     end
%     pause(0.1)
%     saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/GoodfilesFePlots/FIG/%sFe372nm%04d%s%02dGoodNew.fig',station,station,year,smonth(month,:),day(1)));
%     print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/GoodfilesFePlots/EPS/%sFe372nm%04d%s%02dGoodNew',station,station,year,smonth(month,:),day(1)));
%     print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/GoodfilesFePlots/JPG/%sFe372nm%04d%s%02dGoodNew',station,station,year,smonth(month,:),day(1)));
%     
%     
%     
%     scrsz = get(0,'ScreenSize');
...
%         title(sprintf('374-nm Fe Layers on %d-%d %s %04d (UT) ',day(1), day(end),smonth(month,:),year),'fontsize',18)
%     end
%     saveas(gcf,sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/GoodfilesFePlots/FIG/%sFe374nm%04d%s%02dGoodNew.fig',station,station,year,smonth(month,:),day(1)));
%     print('-depsc2',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/GoodfilesFePlots/EPS/%sFe374nm%04d%s%02dGoodNew',station,station,year,smonth(month,:),day(1)));
%     print('-djpeg',sprintf('/Users/asta10/Desktop/Research/Results/%s/OverviewStatistics/GoodfilesFePlots/JPG/%sFe374nm%04d%s%02dGoodNew',station,station,year,smonth(month,:),day(1)));
% 
% end
